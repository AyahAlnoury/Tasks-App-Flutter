...
class ComplatedTasksPage extends StatelessWidget {
  ComplatedTasksPage({
    Key? key,
  }) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<TasksBloc, TasksState>(
      builder: (context, state) {
        List<TaskModel> taskList = state.complatedTasks;
        return Scaffold(
          appBar: AppBarWidget(context, taskList),
          drawer: MyDrawerPage(),
          body: BodyWidget(taskList),
        );
      },
    );
  }

  Column BodyWidget(List<TaskModel> taskList) {
    return Column(
